...
export const File: React.FC<FileProps> = ({ name, className }) => (
  <div
    className={cn(
      "flex items-center space-x-2 py-1 text-sm hover:bg-muted rounded px-2 -translate-x-2 cursor-pointer",
      className,
    )}
  >
...
const FolderTrigger: React.FC<{
  name: string;
  className?: string;
  hasFiles: boolean;
}> = ({ name, className, hasFiles }) => (
  <AccordionPrimitive.Header className="flex">
    {hasFiles ? (
      <AccordionPrimitive.Trigger
        className={cn(
          "flex flex-1 items-center py-1 text-sm font-medium transition-all hover:no-underline hover:bg-muted rounded px-2 -translate-x-2 group",
          className,
        )}
      >
      ...
      <div
        className={cn(
          "flex flex-1 items-center py-1 text-sm font-medium hover:bg-muted rounded px-2 -translate-x-2",
          className,
        )}
      >
...
const FolderContent: React.FC<{
  ...
  <AccordionPrimitive.Content
    className={cn(
      "pl-2 overflow-hidden text-sm data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down",
      className,
    )}
  >
